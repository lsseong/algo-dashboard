...
class BarChart extends Component {
  ...
  drawAxes = (data) => {
    ...
    function make_x_gridlines(xRange) {
      return d3.axisBottom(xRange).ticks(7);
    }
    ...
    function make_y_gridlines(yRange) {
      return d3.axisLeft(yRange).ticks(5);
    }
    ...
    this.svg
      ...
      .call(
        make_y_gridlines(yAxisRange).tickSize(-this.screen_width).tickFormat("")
      )
      .style("color", "lightgrey")
      .style("stroke-width", 0.5);
  ...
  drawBar = (data) => {
    ...
    this.svg
      ...
      .attr(
        "width",
        (d, i) => Math.abs(this.scaler(max, d.position)) * X_UNIT_LENGTH
      )
      .style("stroke", "black")
  ...
  componentDidMount() {
    this.drawChart(this.props.data, this.graphID);
  }
