...
class TradingChart extends Component {
  ...
  initChart = (id, data = [], signaldata = [], analytics = []) => {
    ...
    //update chart
    this.updateChart(this.svg, data, analytics, signaldata);
  };
  ...
  updateChart = (svg, data, analytics, signaldata) => {
    //get Min and Max Value to declare domain and range for x and y axis
    let haveItem = false;
    if (data !== undefined && data !== null && data.length !== 0) {
      this.maxDate.set("data", this.getMaxObjectValue(data, "time"));
      this.minDate.set("data", this.getMinObjectValue(data, "time"));

      this.maxY.set("data", this.getMaxObjectValue(data, "high"));
      this.minY.set("data", this.getMinObjectValue(data, "low"));

      this.maxItem.set("data", data.length);
      haveItem = true;
    }
    if (
      analytics !== undefined &&
    ...
    if (
      ...
      this.maxDate.set("signal", this.getMaxObjectValue(signaldata, "time"));
      this.minDate.set("signal", this.getMinObjectValue(signaldata, "time"));
    }
    //init Axes
    ...
    if (haveItem === true) {
      ...
      this.updateSignal(svg, signaldata, this.xAxisRange);
    }
  };
  ...
  updateData = (svg, data, xAxisRange, yAxisRange, xBandScale) => {
    if (data !== undefined && data !== null && data.length !== 0) {
      //Draw Closing Price Guide Lines and Label
      // console.log('have candles')
      this.drawClosePriceGuides(svg, data, yAxisRange, "open", "close");
      ...
      this.updateOhlcPanel(svg, data);
    }
  };

  updateSignal = (svg, signaldata, xAxisRange) => {
    if (
      signaldata !== undefined &&
      signaldata !== null &&
      signaldata.length !== 0
    ) {
      this.drawSignalPoints(svg, signaldata, xAxisRange, "time", "signal");
    }
  };
  ...
  drawClosePriceGuides = (svg, data, yRange, openName, closeName) => {
    //exit, remove
    svg.selectAll(".price_guides").remove();

    //define group and join
    //enter
    var guidelines_group = svg.append("g").attr("class", "price_guides");
    //append - as many items as you need
    guidelines_group.append("rect").attr("class", "price_bg");
    guidelines_group.append("text").attr("class", "price_text");
    guidelines_group.append("line").attr("class", "price_line");

    let rectboxHeight = 20;

    //guide price rectangle box
    guidelines_group
      .select(".price_bg")
      .attr("x", this.screen_width)
      .attr("y", yRange(data[data.length - 1][closeName]) - rectboxHeight / 2)
      .attr("height", rectboxHeight)
      .attr("width", this.margin.right * 0.8)
      .style(
        "fill",
        data[data.length - 1][openName] === data[data.length - 1][closeName]
          ? "silver"
          : data[data.length - 1][openName] > data[data.length - 1][closeName]
          ? "#ff6961"
          : "#58bf58"
    ...
    //guide price for close
    guidelines_group
      .select(".price_text")
      .attr("x", this.screen_width + this.margin.right * 0.4)
      .attr("y", yRange(data[data.length - 1][closeName]))
      ...
      .text(data[data.length - 1][closeName]);

    //guide lines for close price
    guidelines_group
      .select(".price_line")
      .style("stroke-dasharray", "3, 3")
      .attr("x1", 0)
      .attr("x2", this.screen_width)
      .attr("y1", yRange(data[data.length - 1][closeName]))
      .attr("y2", yRange(data[data.length - 1][closeName]))
      .style(
        "stroke",
        data[data.length - 1][openName] === data[data.length - 1][closeName]
          ? "silver"
  ...
  drawCandleSticks = (
    ...
    svg.selectAll(".candlesticks").remove();

    var candlesticks_group = svg
      .append("g")
    ...
    var candlesticks_enter = candlesticks_group
      .enter()
      .append("g")
      .attr("class", "candles");

    //append - as many items connected to each row in data as you want
    candlesticks_enter.append("rect").attr("class", "bar");
    candlesticks_enter.append("line").attr("class", "stems");

    // candlesticks_enter.append("line").attr("class","data_group");
    //merge
    candlesticks_group = candlesticks_group.merge(candlesticks_enter);
    ...
    candlesticks_group
      ...
      .attr("class", "candle")
      .attr("y", (d) => yRange(Math.max(d.open, d.close)))
      .attr("width", xBandwidth)
      .attr("height", (d) =>
        d[openName] === d[closeName]
          ? 1
          : yRange(Math.min(d[openName], d[closeName])) -
            yRange(Math.max(d[openName], d[closeName]))
      )
      .attr("fill", (d) =>
        d[openName] === d[closeName]
          ? "silver"
    ...
    candlesticks_group
      .select(".stems")
      .attr("x1", (d, i) => xRange(d[xValueName]))
      .attr("x2", (d, i) => xRange(d[xValueName]))
      .attr("y1", (d) => yRange(d[highName]))
      .attr("y2", (d) => yRange(d[lowName]))
      .attr("stroke", (d) =>
        d[openName] === d[closeName]
          ? "white"
          : d[openName] > d[closeName]
  ...
  updateOhlcPanel = (svg, data) => {
    svg.selectAll(".ohlc_panel").remove();

    //define group and join
    //enter
    var ohlc_panel_group = svg.append("g").attr("class", "ohlc_panel");
    //append - as many items as you need
    ohlc_panel_group.append("rect").attr("class", "rect_panel");
    ...
    ohlc_panel_group
      ...
      .attr("x", 0)
      .attr("y", 0)
      .attr("height", this.screen_height * 0.08)
      .attr("width", this.screen_width * 0.4)
      .style("fill", "black")
      .style("opacity", 0.2);

    // price for OHLC
    let panelLabelText = ["O", "H", "L", "C"];
    let panelLabelValue = [
    ...
    //create scaleband for OHLC
    var pointScaleText = d3
      .scaleBand()
      .domain(panelLabelText)
      .range([0, this.screen_width * 0.4]);

    //add items to panel

    ohlc_panel_group
      .append("g")
      .attr("class", "text panel")
      .selectAll("text")
      .data(panelLabelValue)
      .enter()
      .append("text")
      .attr(
        "x",
        (d, i) =>
          pointScaleText(panelLabelText[i]) + pointScaleText.bandwidth() / 2
      )
      .attr("y", this.screen_height * 0.04)
      .style(
        "fill",
      ...
      .style("text-anchor", "middle") // move x axis anchor point to middle
      .style("dominant-baseline", "middle") // move y axis anchor point to middle
      .text((d, i) => panelLabelText[i] + ": " + panelLabelValue[i]);
  };
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      prevProps.signalData !== this.props.signalData ||
      ...
      if (
        ...
        this.maxDate.set(
          "data",
          this.getMaxObjectValue(this.props.data, "time")
        );
        this.minDate.set(
          "data",
          this.getMinObjectValue(this.props.data, "time")
        );

        this.maxY.set("data", this.getMaxObjectValue(this.props.data, "high"));
        this.minY.set("data", this.getMinObjectValue(this.props.data, "low"));

        this.maxItem.set("data", this.props.data.length);
        haveItem = true;
      }
      if (
      ...
      if (
        this.props.signalData !== undefined &&
        this.props.signalData !== null &&
        this.props.signalData.length !== 0
      ) {
        this.maxDate.set(
          "signal",
          this.getMaxObjectValue(this.props.signalData, "time")
        );
        this.minDate.set(
          "signal",
          this.getMinObjectValue(this.props.signalData, "time")
        );
        haveItem = true;
      }
      ...
      if (haveItem === true) {
        ...
        if (prevProps.signalData !== this.props.signalData) {
          this.updateSignal(this.svg, this.props.signalData, this.xAxisRange);
        }

        if (prevProps.analytics !== this.props.analytics) {
          console.log("update analytics");
          console.log(this.props.analytics, prevProps.analytics);
          ...
          //   console.log(this.props.analytics, prevProps.analytics);
          //   console.log("same analytics");
        }
      }
    }
  }

  getUniqueID = (prefix = "tc") => {
    //generate uniqueID
    var length = 10;
    var result = "";
    var characters =
...
export default withStyles(styles)(TradingChart);
