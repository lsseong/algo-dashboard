...
export default class QuoteTable extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    // clear array if strategy changed
    if (this.props.currentStrat !== nextProps.currentStrat) {
      this.storequotearr = [];
    }

    // get the index of the security quote if one already exists
    var pt = this.storequotearr.findIndex(
      i => i.security === nextProps.type.security
    );

    if (this.props.type !== nextProps.type) {
      if (pt !== -1) {
        // quote for an existing security, replace existing one
        this.storequotearr.splice(pt, 1, nextProps.type);
      } else if (pt < 0) {
        // new security quote, add it to the front
                ...
                },
                {
                  ...
                  getProps: (state, rowInfo, column) => {
                    if (rowInfo)
                      return {
                        style: {
                          color: rowInfo.row.bidPrice >= 0 ? "#03c03c" : "red"
                        }
                      };
                    else
                      return {
                ...
                },
                {
                  ...
                  getProps: (state, rowInfo, column) => {
                    if (rowInfo && rowInfo.row)
                      return {
                        style: {
                          color: rowInfo.row.askPrice >= 0 ? "#03c03c" : "red"
                        }
                      };
                    else
                      return {
