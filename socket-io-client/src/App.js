...
//when clicked connection
  handleConnection=(event)=>{
    ...
    if(this.state.connectstatus === "Connect"){
      ...
      setInterval(this.initConnection(this.state.host,this.state.port), 1000);
  ...
  render(){ 
    ...
    return (
      ...
      <div className="col-md-6">
          ...
          <form onSubmit={this.handleConnection.bind(this)} className="form-control-sm">
            <div className="form-row">
            <div className="col">
              <input type="text" name="host" className="form-control form-control-sm" placeholder="Host" value = {this.state.host} onChange={this.connectionInput} disabled = {(this.state.disabled)? "disabled" : ""} required/>
           </div>
         <div className="col">
         
              <input type="text" name="port" className="form-control form-control-sm" placeholder="Port" value = {this.state.port} onChange={this.connectionInput} disabled = {(this.state.disabled)? "disabled" : ""}required/>
              
          </div>
          <div className="col">
          <button type="submit" className="btn btn-outline-primary btn-sm btn-block" value ={this.state.connectstatus}  >{this.state.connectstatus}</button>
          </div>
          </div>
